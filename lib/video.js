...
module.exports = function (filePath, settings, infoConfiguration, infoFile) {
	...
	this.setVideoFormat = function (format) {
		...
		if (this.info_configuration.containerEncode.indexOf(format) != -1) {
			if (options.video == undefined)
				options.video = new Object();
			// Set the new option
	...
	this.setVideoCodec = function (codec) {
		...
		if (this.info_configuration.encodeVideo.indexOf(codec) != -1) {
			if (options.video == undefined)
				options.video = new Object();
			// Set the new option
	...
	/**
	 * Set the transpose mode of the video, only accepted values are 90,-90 (clockwise angle)
	 */
	this.setVideoTranspose = function (angle) {
		if (options.video == undefined)
			options.video = {}
		// Set the new option
		if (angle == 90)
			options.video.transpose = 1
		if (angle == -90)
			options.video.transpose = 2
		return this;
	}
	...
	this.setAudioCodec = function (codec) {
		// Check if the codec is supported by ffmpeg version
		if (this.info_configuration.encodeAudio.indexOf(codec) != -1) {
	...
	/**
	 * Sets the flag for streaming mode optimization
	 * 
	 * (Headers in front, etc...)
	 */
	this.setStreamingMode = function (streamingMode) {
		if (!options.container)
			options.container = {}

		if (streamingMode)
			options.container.streaming = true
		else
			delete options.container.streaming
	}
	 ...
	 */
	this.save = async function (destinationFileName, callback) {
		// Overwrite by default (otherwise will hang on file exists!)
		this.addCommand('-y')
		// Check for container specific options
		if (options.hasOwnProperty('container')) {
			if (options.container.hasOwnProperty('streaming')) {
				/*
				The -movflags faststart option is essential for streaming, since it moves the container metadata
				to the beginning of the file instead of leaving it at the end. This will allow the playback to
				immediately start instead of having to wait for the file to be loaded entirely.
				*/
				this.addCommand('-movflags', 'faststart')

				if (options.hasOwnProperty('video') && options.video.hasOwnProperty('codec') && options.video.codec != 'copy') {
					this.addCommand('-profile:v', 'main')
				}
			}
		}
		...
		if (options.hasOwnProperty('video')) {
			...
			if (options.video.hasOwnProperty('disabled')) {
				...
				if (options.video.hasOwnProperty('codec'))
					this.addCommand('-c:v', options.video.codec)
				...
				if (options.video.hasOwnProperty('framerate'))
					this.addCommand('-vf', "fps=" + options.video.framerate)
				...
				if (options.video.hasOwnProperty('transpose'))
					this.addCommand('-vf', 'transpose=' + options.video.transpose)
		...
		if (options.hasOwnProperty('audio')) {
			...
			if (options.audio.hasOwnProperty('disabled')) {
				...
				if (options.audio.hasOwnProperty('codec'))
					this.addCommand('-c:a', options.audio.codec);
	...
	var _calculateNewDimension = function () {
		...
		if (fixedWidth) {
			...
			// Check if the video has the aspect ratio setted
			if (!utils.isEmptyObj(this.metadata.video.aspect)) {
				if (this.metadata.video.rotate == 90) {
					width = Math.round((height / this.metadata.video.aspect.x) * this.metadata.video.aspect.y);
				} else {
	 ...
	 */
	var execCommand = async function (folder) {
		var i;
		...
		let result = await utils.exec(finalCommands, settings)
		if (result.error) {
			throw result.error
		}
	 ...
	 */
	this.fnExtractSoundToMP3 = async function (destionationFileName, callback) {
		...
		// Executing the commands list
		return await execCommand(callback)
	}
	...
	this.fnExtractFrameToJPG = function (/* destinationFolder, settings, callback */) {
		...
		// Get the dimensions
		var newDimension = _calculateNewDimension();
