...
const ffmpeg = function (/* inputFilepath, settings, callback */) {
	 ...
	 */
	const _ffmpegInfoConfiguration = async function (settings) {
		...
		let stdout = (await utils.exec([ffmpeg.bin, '-formats', '2>&1'], settings)).stdout
		...
		// Get the codec list
		const formatList = stdout.match(/ (DE|D|E) (.*) {1,} (.*)/g)
		// Scan all codec
		for (const f of formatList) {
			// Get the match value
			const match = / (DE|D|E) (.*) {1,} (.*)/.exec(f)
			...
			if (match) {
				...
				if (scope == 'D' || scope == 'DE') format.containerDecode.push(extension)
				if (scope == 'E' || scope == 'DE') format.containerEncode.push(extension)
			}
		}

		// Now we get the codecs (not to be confused with formats)
		stdout = (await utils.exec([ffmpeg.bin, '-codecs', '2>&1'], settings)).stdout
		// Get the codec list
		const codecsList = stdout.match(/ (D\.|\.E|DE).{4} (.*) {1,} (.*)/g)
		// Scan all codec
		for (const e of codecsList) {
			// Get the match value
			var match = / (D\.|\.E|DE)(.){1}.{3} (.*) {1,} (.*)/.exec(e)
			// Check if match is valid
			if (match) {
				// Get the value from the match
				const scope = match[1].replace(/\s/g, '')
				const type = match[2]
				const extension = match[3].replace(/\s/g, '')
				// Check which scope is best suited
				if (scope == 'D.' || scope == 'DE') {
					switch (type) {
						case 'A':
							format.decodeAudio.push(extension)
							break
						case 'V':
							format.decodeVideo.push(extension)
							break
						case 'S':
							format.decodeSubtitle.push(extension)
							break
					}
				}
				if (scope == '.E' || scope == 'DE') {
					switch (type) {
						case 'A':
							format.encodeAudio.push(extension)
							break
						case 'V':
							format.encodeVideo.push(extension)
							break
						case 'S':
							format.encodeSubtitle.push(extension)
							break
					}
				}
			}
		}
		format.encodeAudio.push('copy')
		format.encodeVideo.push('copy')
		format.encodeSubtitle.push('copy')
		return format;
	}
	 ...
	 * Get the video info
	 */
	var _videoInfo = async function (fileInput, settings) {
		// Make the call to retrieve information about the ffmpeg
		const exData = await utils.exec([ffmpeg.bin, '-i', utils.addQuotes(fileInput), '2>&1'], settings)
		const stdout = exData.stdout
	...
	var _getInformation = async function (fileInput, settings) {
		// Ensure that we can execute FFMPEG
		try {
			await utils.exec([ffmpeg.bin, '-version'])
		} catch (e) {
			console.log(e)
			return Promise.reject('Cannot find "' + ffmpeg.bin + '", please ensure the binary path is correct!')
		}
		var deferreds = [];
		// Add promise
